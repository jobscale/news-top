...
{"credibility":信頼性・具体性,"influence":["影響範囲"],"sentimental":感情値}
- 信頼性・具体性：float 0.0〜5.0 の範囲
- 影響範囲：最大４つまで 例 怪我,疾患,患者,建物,産業,株価,経済,歴史,未来,過去,地球,快方,好転,縮小,犯罪,芸能,スポーツ,エンタメ,政治,個人,家族,戦争,都道府県,市区町村
- 感情値：float ネガティブ -2 〜 ポジティブ 2 の範囲
説明や理由やは不要です。
`;
...
export const calcScore = async title => {
  ...
  .then(answer => {
    ...
    const sum = { cheat: 0 };
    sum.cheat += bewitch.filter(word => answer.influence?.includes(word)).length;
    return {
      ...answer, ...sum,
    };
  })
